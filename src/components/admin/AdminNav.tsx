...
const links: { href: string; label: string; scope: AdminScope }[] = [
  ...
  { href: "/admin/catalog", label: "Catalog", scope: "catalog" },
  {
    href: "/admin/feature-controls",
    label: "Feature controls",
    scope: "security_break_glass",
  },
...
export function AdminNav({ scopes }: { scopes: string[] }) {
  ...
  return (
    <nav
      aria-label="Admin"
      className="flex flex-col gap-1 border-b border-[var(--color-border)] bg-white px-4 py-3 md:flex-row md:items-center md:gap-4"
    >
      <Link
        href="/discover"
