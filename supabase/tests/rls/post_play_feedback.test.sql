...
values
  ...
  (:'user_b', 'fb-b@test.local', crypt('test', gen_salt('bf')), now(), now(), now(), '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated')
...
insert into public.gaming_sessions (
  ...
  status, confirmed_start_at, session_length_minutes,
  participant_a_id, participant_b_id, completed_at
)
select pi.id, pi.conversation_id, pi.game_id, pi.platform_id,
  'completed', now() - interval '1 day', 60,
...
select lives_ok(
  $$
  insert into public.post_play_feedback (session_id, account_id, continuation)
  select gs.id, a.id, 'play_again'::public.post_play_continuation
  from public.gaming_sessions gs
  join public.accounts a on a.auth_user_id = 'c5222222-2222-2222-2222-222222222222'::uuid
  limit 1
  $$,
  'can insert own feedback'
);
